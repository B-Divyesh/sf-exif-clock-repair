...
let status = stored.recovered
  ...
  : records.length ? `Restored ${records.length} previous finding${records.length === 1 ? '' : 's'}. Photo bytes are not stored.`
  ...
  app.innerHTML = `
    ...
    <main id="main">
      ...
      ${records.length ? `<section class="workspace" aria-labelledby="findings-title">
        ...
        <div class="method" role="note"><strong>How to read this:</strong> a proposal does not change a photo. We compare documented EXIF fields and use the file date only to reveal exact whole-hour patterns. The ZIP keeps each sidecar beside its original folder path. Spot-check before importing.</div>
      ...
      </section>` : `<section class="empty-note"><div class="index">01</div><div><h2>Start with a copy of the archive</h2><p>Exif Clock Repair reads JPEG EXIF in this version. It saves repair-plan metadata in this browser, not photo bytes. Original files stay unchanged.</p></div><ol><li>Choose a folder or a selection of JPEGs.</li><li>Review grouped conflicts and whole-hour clues.</li><li>Export JSON/XMP sidecars and an audit ledger.</li></ol></section>`}
    ...
    </main><footer><span>Plan JPEG capture-time repairs locally.</span><span>Local use · no analytics · <a href="/privacy/">Privacy</a> · <a href="/terms/">Terms</a> · Built by Param Factory · v1.1.0</span></footer><div id="update" class="toast" hidden>Update ready. <button id="reload">Reload</button></div>`;
  bind();
}
...
function bind() {
  ...
  document.querySelector<HTMLAnchorElement>('.demo-banner a[href="/"]')?.addEventListener('click', () => localStorage.removeItem(storageKey));
